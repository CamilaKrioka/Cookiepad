...
import RecetaFilter from './RecetaFilter';
...
const ListadoRecetas = (props) => {
  ...
  const cargarListadoRecetas = () => {
    ...
    if (props.type === 'recetas' && props.searchRec) {
      endpoint += '/search/' + props.searchRec;
    ...
    if (props.user) {
      ...
      fetch(`http://localhost:8080/favoritos/${props.user.id}`).then(
        ...
        data => {
          ...
          fetch(`http://localhost:8080/${endpoint}?${filterParams}`).then(
            response => response.json()
          ).then(
            data => {
              setRecetas(data);
  ...
  useEffect(cargarListadoRecetas, [props.user, props.searchRec, props.filtros]);
  ...
  const handleDeleteClick = (idReceta) => {
    ...
    Swal.fire({
      title: 'Confirmar que desea eliminar la receta',
      icon: 'question',
  ...
  return (
    ...
    <>
      ...
      <RecetaFilter onFilterChange={handleFilterChange} />
